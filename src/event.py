...
class Event:
    ...
    def create_from_config(cls, season: str, event_id: str):
        ...
        if "oris_id" in config:
            try:
                oris_info = cls.get_info_from_oris(config["oris_id"])
                config = cls.add_oris_info(config, oris_info)
    ...
    @classmethod
    def get_all_events(cls, season: str, as_dicts: bool = False) -> Dict:
        ...
        for event_file in season_dir.glob("*.json"):
            event = cls.create_from_config(season, event_file.stem)
        ...
        return dict(sorted(events.items(), key=event_date))
